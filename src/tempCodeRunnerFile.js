...
useEffect(() => {
  const fetchProjections = async () => {
    try {
      ...
      const res = await fetch('/data/final_proj.csv');
      ...
      console.log('Parsed data:', parsed.length, 'players');
...
useEffect(() => {
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch('/data/prev_stats.csv');
      ...
      const text = await res.text();
      console.log('Stats CSV text length:', text.length);
      console.log('Stats first 200 chars:', text.substring(0, 200));
      
      const parsed = parseStatsCSV(text);
      console.log('Parsed stats data:', parsed.length, 'players');
      console.log('First stats player:', parsed[0]);
